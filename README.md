...
NOTE: For libraries and modules installation, please kindly refer to the Requirements.txt file in the Individual Assignment 2 Folder to install all the required versions correctly. For some versions like numpy, I   am using an older version for compatability with other libraries like torch which only supports numpy 1.x versions and not numpy 2.x versions.
